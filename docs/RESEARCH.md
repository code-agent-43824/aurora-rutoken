...
**Практический вывод для приложения:** имена считывателей не выдумываются и не
зашиваются в код — они читаются с устройства через `PP_ENUMREADERS`, и форма
создания предлагает то, что на устройстве действительно есть. Режим каждого
считывателя определяется по маркеру в имени носителя тем же правилом, что уже
используется при чтении (`fkc` → ФКН, `pkcs11` → активный токен, иначе CSP).
