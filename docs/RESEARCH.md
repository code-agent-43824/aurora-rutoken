...
Чего этот пример **не** показывает (ждём следующего замера владельца):

- как выглядит та же пара, когда на ней появится сертификат;
- как называется контейнер, созданный самим КриптоПро в режиме CSP или ФКН
  (там `CKA_ID` ключевой пары PKCS#11 может отсутствовать как понятие).
