...
|---|---|---|
| Пассивный, «режим CSP» | `rutoken_…`, `rutoken_lt_…` | контейнер создаёт программный провайдер КриптоПро; ключи бывают экспортируемыми |
| Активный токен | `pkcs11_rutoken_…` | ключи в криптоядре Рутокена, операции выполняет устройство, ключ неизвлекаем |
| ФКН | `rutoken_fkc_…`, по NFC `rutoken_fkc_nfc_…` | неизвлекаемые ключи + защита канала по SESPAKE; отдельные пароль и PUK ФКН |
