...
Это подтверждает на реальном устройстве базовый путь USB `Рутокен → CCID/pcscd → libpcsclite → приложение`. Фактическое появление Рутокен NFC как PC/SC-ридера ещё нужно зафиксировать отдельным прогоном с приложенным NFC-токеном.
