...
// Полное (FQCN) имя контейнера открытого контекста. Значение стандартное
// (wincrypt.h, PP_UNIQUE_CONTAINER). Нужно, чтобы ПОСЛЕ создания прочитать, на
// какой носитель контейнер лёг на самом деле, и не выдавать выбранный режим за
// полученный: чтение идёт по уже открытому дескриптору, лишних обращений к
// носителю нет.
static const Dword PpUniqueContainer = 36;
