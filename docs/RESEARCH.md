...
Чего эти два примера **не** показывают (ждём следующего замера владельца):

- как называется контейнер, созданный **самим КриптоПро** (токен будет очищен);
- как называется контейнер режима CSP или ФКН — там ключевой пары PKCS#11 может
  не быть вовсе, и правило `ID_<hex>` к ним, скорее всего, неприменимо.
